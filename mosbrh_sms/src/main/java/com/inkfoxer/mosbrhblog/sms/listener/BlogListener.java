...
import com.inkfoxer.mosbrhblog.commons.feign.SearchFeignClient;
import com.inkfoxer.mosbrhblog.commons.feign.WebFeignClient;
import com.inkfoxer.mosbrhblog.sms.global.RedisConf;
import com.inkfoxer.mosbrhblog.sms.global.SysConf;
...
import com.inkfoxer.mosbrhblog.base.enums.ESearchModel;
...
public class BlogListener {
    ...
    @RabbitListener(queues = "mosbrh.blog")
        ...
        if (map != null) {
            ...
            try {
                switch (comment) {
                    ...
                    case SysConf.DELETE: {
                        log.info("mosbrh-sms处理删除博客: uid:" + uid);
                        updateSearch(map);
                        if (ESearchModel.ES.equals(searchModel)) {
                            // 增加ES索引
                    ...
                    break;
                    default: {
                        log.info("mosbrh-sms处理博客兜底方法");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
                log.error("【mosbrh-sms】出现异常，请查看mosbrh-search是否启动！searchModel: " + searchModel);
            }
        }
    }
